...
code = code.replace(/if \(intersectX && intersectY && mob\.type === 'rival_thug'\)/, "if (intersectX && intersectY)");
code = code.replace(/const mTop = mob\.y - 24;/g, "const mTop = mob.type === 'zombie' ? mob.y - 24 : mob.y - 12;");

const newMobRender = `         if (mob.type === 'zombie') {
...
code = code.replace(/         if \(mob\.type === 'rival_thug'\) \{[\s\S]*?         \}/, newMobRender);
