      ...
      const castAbility = (abilityName: string) => {
          ...
          if (cd > 0) {
              ...
              if (abilityName === 'slash') {
                  ...
                  state.socket.emit('use_ability', { ability: 'dash' });
                  ...
                  state.socket.emit('fire_projectile', { type: 'trap', x: gameState.current.player.x, y: gameState.current.player.y, vx: 0, vy: 0, damage: 25 + (propsRef.current.skills?.dexterity || 0) * 3 });
              }
          }
      };
...
if start_idx != -1:
    end_idx = start_idx
    while "if (key === ' ' || key === 'ArrowUp' || key === 'w') {" not in lines[end_idx]:
        end_idx += 1
